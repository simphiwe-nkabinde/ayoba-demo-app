...
export function App() {
  ...
  useEffect(() => {
    if (!gameId || !userJid) {
      setError(true);
      setErrorMessage('game ID or player ID not found from query params');
    } else {
      setLoading(true);
      fetch(`http://localhost:1337/api/games?filters[uid][$eq]=${gameId}&populate=*`)
        .then(res => res.json())
        .then(data => {
          if (!data.data[0].attributes.playerScores.data.length) {
            setError(true)
            setErrorMessage('no scores available for this game')
          }
  ...
  function showErrorMessage() {
    return (
      <div class={`text-center bg-light rounded-4 h-100 py-5`}>
        <p class='my-3 text-body-tertiary'>Erorr Loading Leaderboard</p>
          ...
          {!loading &&
            <div class='bg-light ps-2 pe-2 pt-2 pb-0 rounded-top-4 d-flex flex-column justify-content-between'>
              ...
              <div class='mb-4'>
                ...
                <ScoreCard
                  scoreData={scores.find(score => score.attributes.jid == userJid)?.attributes}
                  isUserScore={true}
                  color='blue3' className='rounded-3' />
              </div>
